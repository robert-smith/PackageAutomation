function New-PAIntuneWin32AppPackage {
    <#
    .SYNOPSIS
            ...
            if (Test-Path -Path (Join-Path -Path $SourceFolder -ChildPath $SetupFile)) {
                ...
                if (Test-Path -Path $OutputFolder) {
                    ...
                    if (Test-Path -Path $IntuneWinAppUtilPath) {
                        ...
                        $PackageInvocation = Invoke-Executable -FilePath $IntuneWinAppUtilPath -Arguments "-q -c ""$($SourceFolder)"" -s ""$($SetupFile)"" -o ""$($OutPutFolder)""" -Quiet $Quiet # -q
                        if ($PackageInvocation -eq 0) {
                            ...
                            if (Test-Path -Path $IntuneWinAppPackage) {
                                ...
                                $IntuneWinAppMetaData = Get-PAIntuneWin32AppMetaData -FilePath $IntuneWinAppPackage
