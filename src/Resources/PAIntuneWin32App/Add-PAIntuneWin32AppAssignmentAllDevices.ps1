function Add-PAIntuneWin32AppAssignmentAllDevices {
    <#
    .SYNOPSIS
    ...
    Process {
        ...
        if ($Win32App -ne $null) {
            ...
            $DuplicateAssignment = Test-PAIntuneWin32AppAssignment -ID $Win32AppID -Target "AllDevices"
            if ($DuplicateAssignment -eq $false) {
                try {
                    ...
                    Write-Error -Message "An error occurred while creating a Win32 app assignment: $($TargetFilePath). Error message: $($_.Exception.Message)"
                }
            }
        }
        else {
